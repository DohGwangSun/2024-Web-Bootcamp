...
app.get("/random", (req, res) => {
  const radomIndex = Math.floor(Math.random() * jokes.length);
  res.json(jokes[radomIndex]);
});
...
app.get("/filter", (req, res) => {
  ...
  const filteredActivties = jokes.filter((joke) => joke.jokeType === type);
  res.json(filteredActivties);
  }
); 
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res
      .status(404)
      .send({ error: `joke with id ${id} not found. No jokes were deleted.` });
  }
});
